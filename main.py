...
from src import DelayDiscountingController, run_trial
...
def _run_impl(*, mode: str, output_dir: Path | None, cfg: dict, participant_id: str):
    ...
    if getattr(settings, "voice_enabled", False) and mode not in ("qa", "sim"):
    ...
    controller = DelayDiscountingController.from_dict(settings.controller)
